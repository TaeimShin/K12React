...
export default로 지정한 이름을 그대로 사용하면된다. 
...
class App extends Component {
  ...
  render(){
    ...
    else if (this.state.mode ==='read')
    {
      ...
      //create일때는 입력폼이 보인다.
      //자식 컴포넌트에서 폼값을 받기위해 onSubmitValue라는 props내려준다
      _article = <CreateForm onSubmitValue={function(_title,_desc){
        console.log(_title, _desc);
        //일련번호 부여를 위해 +1 증가
        this.max_content_id = this.max_content_id +1;
        //일련번호와 폼값을 이용해서 새로운 객체를 추가한다.
        //concat()함수는 배열을 추가하는 기능도 가지고있다.
        var _contents = this.state.contents.concat(
          {id:this.max_content_id, title:_title, desc:_desc}
        );
        //state값을 변경한다. 앞에서 추가한 배열로 교체된다. 그리고 화면이 다시 랜더링 된다.
        this.setState({
          contents : _contents,
          mode :'read',
      ...
      */
      ...
      _article = <UpdateForm readData={_readData} 
        ...
        /*
        삭제를 여기에서 처리하면 렌더링이 2번일어나서 비효율
        밑에는 btn_mode를 딜리트로 바꾼담에 렌더링을 하니까
        */
    }
    ...
    return (
      <div className="App">
        ...
        */ }
        ...
        {/* Navi컴포넌트로 state변경 기능을 가진 함수를 props로 내려준다.
        ...
        <Buttons onChangeMode={function(btn_mode){
          ...
          if(btn_mode==='delete'){
            //리엑트에서 confirm을 사용할때는 반드시 window를 붙여야 한다.
            ...
            //mode 가 delete가 아닌경우 state변경 후 렌더링한다.
            this.setState({
              mode : btn_mode
            });
          }
