        ...
        return(
            <article>
                ...
                <form action="/create_process" method="post" onSubmit={function(e){
                    ...
                    {/* 기존게시물에 대한 수정이므로 게시물 아이디를 저장할 hidden 필요 */}
                    {/* 
                    App.js에서 기존 게시물을 props로 전달한 후 
                    input상자에 즉시 삽입하면 값을 변경하는것이 불가능하다. props는 읽기전용
                    데이터이므로 값의 변경은 state를 통해서 가능하다.
                    이 경우 props로 전달된 데이터를 통해 state를 초기화하고, 각 input
                    상자에서 변경되는 값은 onChange이벤트를 통해 동기화 해야 한다. */}
                    <input type="hidden" name="id" value={this.props.readData.id} />
                    <p><input type="text" name="title" placeholder="제목입력"
                        value={this.state.title} onChange={this.inputChangeHandler}></input></p>
                    <p><textarea name="desc" placeholder="내용입력"
                        value={this.state.desc} onChange={this.inputChangeHandler}></textarea></p>
